...
export default async function SettingsPage({
  ...
  if (!session) {
    return (
      <main className="mx-auto max-w-2xl px-6 py-16">
        <p className="text-hearth-ink">Sign in to manage your settings.</p>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-16">
      <h1 className="font-display text-2xl text-hearth-ink">Settings</h1>

      {notice && (
        <p className="mt-4 rounded-md border border-verdigris-sky px-4 py-3 text-sm text-verdigris-sky">
          {notice}
        </p>
      )}
      {error && (
      ...
      <section className="mt-8 rounded-md border border-ash-stone bg-cloudlight p-5">
        <h2 className="font-display text-lg text-hearth-ink">Profile</h2>
        <p className="mt-1 text-sm text-hearth-ink/60">{me.email}</p>
      ...
      {manageOrg && org && (
        <>
          <section className="mt-8 rounded-md border border-ash-stone bg-cloudlight p-5">
            <h2 className="font-display text-lg text-hearth-ink">Workspace</h2>
            <p className="mt-1 text-xs text-hearth-ink/60">/{org.slug}</p>
          ...
          <section className="mt-8 rounded-md border border-ash-stone bg-cloudlight p-5">
            <h2 className="font-display text-lg text-hearth-ink">Domains &amp; auto-join</h2>
            ...
            <ul className="mt-4 flex flex-col gap-3">
              {domains.map((d) => (
                <li key={d.id} className="rounded-md border border-ash-stone bg-white p-3 text-sm">
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="font-medium text-hearth-ink">{d.domain}</span>
                    <span
                      className={`rounded px-2 py-0.5 text-xs ${
                        d.verifiedAt ? "bg-verdigris-sky text-cloudlight" : "bg-ash-stone text-hearth-ink"
                      }`}
                    >
                    ...
                    {!d.verifiedAt && (
                      <form action={verifyOrgDomain}>
                        ...
                        <button type="submit" className="text-xs text-verdigris-sky underline">
                          Check DNS
                        </button>
                      </form>
                    )}
                  ...
                  {!d.verifiedAt && d.dns && (
                    <dl className="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 font-mono text-xs text-hearth-ink/80">
            ...
            <form action={addOrgDomain} className="mt-4 flex flex-wrap items-end gap-3">
              ...
              <button type="submit" className="rounded-md border border-ash-stone px-4 py-2 text-sm text-hearth-ink">
                Add
              </button>
            </form>

            <form action={setAutoJoin} className="mt-6 flex flex-wrap items-end gap-3 border-t border-ash-stone pt-4">
              <label className="flex flex-1 flex-col gap-1 text-sm text-hearth-ink">
                Auto-join domain
                <select
                  name="autoJoinDomain"
                  defaultValue={org.autoJoinDomain ?? ""}
                  className="rounded-md border border-ash-stone bg-white px-3 py-2"
                >
              ...
              <button
                ...
                className="rounded-md bg-ember-core px-4 py-2 text-sm text-cloudlight disabled:opacity-50"
              >
                Save
              </button>
              {verified.length === 0 && (
                <p className="w-full text-xs text-hearth-ink/60">
